...
class VNStockClient:
    """
    Đóng gói các call tới vnstock, có retry/backoff khi dính rate-limit (SystemExit).
    ...
    def _df_or_empty(self, df: Optional[pd.DataFrame]) -> pd.DataFrame:
        """Trả về DataFrame hoặc DataFrame rỗng nếu input là None."""
        if df is None:
            return pd.DataFrame()
        return df

    def _normalize_shareholder_df(self, df: pd.DataFrame) -> pd.DataFrame:
        """Chuẩn hoá DataFrame cổ đông để có cấu trúc thống nhất."""
        ...
        drop_cols = [
            col for col in ("__typename", "ticker", "en__owner_full_name")
    ...
    def _fetch_vci_shareholders_direct(self, symbol: str) -> pd.DataFrame:
        """Fetch trực tiếp cổ đông từ nguồn VCI Explorer."""
        try:
            explorer = VCIExplorerCompany(
                symbol=symbol, random_agent=False, to_df=True, show_log=False
            )
    ...
    def _fetch_shareholders(self, symbol: str, *companies: VNCompany) -> pd.DataFrame:
        """Fetch danh sách cổ đông từ VCI hoặc TCBS."""
    ...
    # ------------------------------------------------------------
    # Fetch thông tin công ty (bundle)
    # ------------------------------------------------------------
        ...
        while retries <= self.max_retries:
            ...
            try:
                ...
                bundle = {
                    ...
                    "shareholders_df": self._fetch_shareholders(
                        symbol, vn_company_vci, vn_company_tcbs
                    ),
                ...
                print(
                    ...
                    f"Retry {retries}/{self.max_retries} after {wait_time}s..."
                )
    ...
    # ------------------------------------------------------------
    # Phiên bản safe (chịu lỗi)
    # ------------------------------------------------------------
    def fetch_company_bundle_safe(
        self, symbol: str
    ) -> Tuple[Dict[str, pd.DataFrame], bool]:
        """
        Safe/robust variant của fetch_company_bundle với rate limiting.
        - Wraps mỗi VNStock call trong try/except riêng.
        - Trả về partial bundle; ok=True nếu có overview từ TCBS.
        """
        retries = 0
        while retries <= self.max_retries:
            listing = None
